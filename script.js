document.addEventListener('DOMContentLoaded', () => {
  const API_KEY = "cur_live_AeeUpu9G1ad8EnyavEegzTqj7GHe9cUYJY47aFUU";
  ...
  btn.addEventListener("click", async (evt) => {
    ...
    try {
      ...
      // Check if the fromCurr and toCurr codes exist in the rates object
      if (!(fromCurr.value in rates) || !(toCurr.value in rates)) {
      ...
      let finalAmount = (amountValue / fromRate) * toRate;
      msg.innerText = `${amountValue} ${fromCurr.value} = ${finalAmount} ${toCurr.value}`;
      amountInput.value="";
    } catch (error) {
      console.error(error);
      msg.innerText = 'Failed to perform currency conversion. Please try again later.';
    }
  });
